...
4. 账户维未回测（holdings.json 只有当下持仓、无历史流水）→ 回测不含 account 维，
   live 引擎含 account 维（±10 分），属已知差异
...
def main() -> int:
    ...
    lines = [
        ...
        "4. 账户维未回测（无历史持仓流水）；live 引擎含账户维（±10），属已知差异",
    ...
    lines += ["", "## 六、结论", "", f"**{verdict_line}**", "",
              ...
              "③账户维缺席；④前复权 K 线按当日口径回放（轻微幸存者偏差已知）；⑤未含申赎费用。</sub>"]
