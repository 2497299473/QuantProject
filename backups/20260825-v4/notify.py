...
def _build_card(slot: str, signals: dict, account: dict, realtime: dict | None = None) -> dict:
        ...
        "card": {
            ...
            "elements": [{"tag": "div", "fields": fields}] + [{
                ...
                "elements": [{"tag": "plain_text",
                              "content": "三因子弱参考（MACD柱趋势/池内排名/距60日新高）· 不构成买卖指令 · 绝不自动下单"}],
            }],
        },
    }


def push_feishu(slot: str, signals: dict, account: dict, realtime: dict | None = None) -> dict:
    env = _load_env()
    ...
    payload = _build_card(slot, signals, account, realtime)
