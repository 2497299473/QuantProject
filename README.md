...
- V4.3.1 收口加固: **冻结核验 + provenance 透传（2026-09-21，外部复审四条落地）**（① KFP 留档解析单一真源 `freeze_verify_tool.load_kfp_record()`：CLI 与库共用侧车发现/meta 回退，终结上轮两处 fallback 漂移；V4.3 scoped 件缺 `kline_fingerprint_*.json` 侧车 ⇒ reject 判 UNKNOWN（meta 恢复不了 universe 码表，退化口径重算要么假 DRIFTED 要么掩盖真漂移——宁可不可比不可猜）。② G-B 判据单一真源 `comparability_state()` 增 **INCOMPLETE** 态：重算侧 unreadable 非空 ⇒ 聚合有空洞，比对不成立（SAME 不可声称、DRIFTED 不可归因）；软标记不拦截，report_line 与 triple JSON 同步带出。③ registry provenance 透传：`resolve_samples()` 的 snap_info 新增 `snapshot_provenance`（snapshot_file + samples_sha256_lf + kfp 留档/重算/状态五元组），train_forecast_model → save_models → register_model 白名单绑定进模型条目——回答"这个模型基于哪份冻结样本+哪套 K 线指纹训练"；不论 G-B 状态照写全（指纹记实际消费之物），FRESH/缺件诚实为 None 绝不伪造。④ `backtest_forecast` / `experiment_state_feature` / `train_forecast_model` 补 report_line 输出（12 入口全覆盖）；`test_pit_guard` 的 PASS 断言补日历缺失 skip guard（bundle 不带运行时行情缓存时诚实降级）。fast 套件 440→447（+7 新测例），重训/重冻前后数字口径不变）
- V4.3.1-⑤ 收口: **冻结链严格化 + registry provenance 门禁（2026-09-22，外部复审跟进三条落地）**（⑤a 显式 `--kfp` 严格模式（`freeze_verify_tool`）：文件不存在 / 不可解析（JSONDecodeError→UNKNOWN，不再裸 traceback）/ 文件名日期标签与样本不符 / scoped 侧车 cutoff+stock_n 与 meta scope 不符 ⇒ 一律判 UNKNOWN，**拒绝回退同目录兄弟/meta 字段**（显式指定=调用方已知该用哪份；回退会静默校验另一份文件 ⇒ 假 PASS / 假 DRIFTED）；自动发现路径与库路径（frozen_dataset，explicit=None）不变，G-B 仍为软标记不改 exit 码。⑤b 冻结 fund 侧 scope（`freeze_samples`）：`build_fingerprint` 另传 `fund_codes`=<样本基金排序去重>，无关基金缓存变化（扩池/新基金引入/净值 TTL 改写）不改冻结指纹；09-10 锚点的 `data/klines` 恰只含 4 只样本基金 ⇒ canonical 逐字节不变，既有锚点数字不回归；kline_fingerprint selftest 23→27（无关基金新增/改写→scoped 不变、在域基金改写仍敏感、默认口径语义不变）。⑤c registry provenance 门禁（`model_registry.verify_approval`）：`snapshot_provenance` 五键齐全（FROZEN）才允许对外展示；FRESH/历史无此块 ⇒ 拒绝（research-only）。门禁**只进授权入口**：--fresh 训练路径不经过 verify_approval 不受影响；`derive_promotion` 五审契约纯函数判据一字不动；v2/v3 本来就 blocked，无回归。fast 套件 597→606（+9 新测例，venv 3.12 OK/skipped=3），重冻后锚点口径不变）
- V4.3.1-⑥ 收口: **LOFO 证据可追溯 + 陈旧标记上链（2026-09-22，实验口径与线上口径一致性风险落地）**（① 证据文件绑定三件套（`backtest_lofo.py`）：产出时写 `status=FRESH` + `valid_for{code_commit / samples_file / samples_sha256_lf / kfp_comparability}`，`code_commit` 复用 `freeze_verify_tool.git_commit()` 单一真源不另起炉灶；报告首行区同步打印有效性行。② 存量 08-30 件就地标 **STALE**（`lofo_evidence.json`，改动前先落 `lofo_evidence.json.bak-20260922`）：`stale_reason` 记因（09-10 重冻结 + 09-18 C2 特征非时点不变），`valid_for` 四键诚实置 null 并附 note——该件生成于 V4.3 P0-1 冻结机制之前、当时消费活缓存无三件套可回填，且实测 `git rev-list --before 2026-08-30T11:27:47+08:00` 返回空（仓库历史不追溯该时点，最早提交 09-08），**不伪造可追溯性**。③ 消费端出陈旧标记（`core/forecast_engine.py` + `core/report_generator.py` + `backtest_t5_scorecard.py`）：`_load_fund_evidence()` 改返回 `(funds, status)`，缺失/损坏/无字段 ⇒ `unknown`（诚实未判定，不猜 FRESH）；`get_fund_evidence()` 增 `status` / `stale` 两个纯增量键；日报「模型可信度」尾随 `⚠证据陈旧`，评分卡 ⑨ 行按 status 显示 `STALE（09-26 须重跑）`。**confidence 数值行为一字不变**——陈旧标记只作展示，不进门禁、不改权重（新增回归闸门测例显式钉住这条：三种 status 下 confidence 必须逐位相等）。④ 措辞对齐事实（`backtest_walk_forward.py`）：Path-WF 注释/报告原写「每折重估**基金级** μ/σ」，实际调用 `fit_path_params(f["train"], 5)` 未传 `fund_code` ⇒ 是**池级** μ + 逐基金 σ 窗等权拼接（v1.3 语义）。改注释与报告口径为「池级 μ/σ（σ 窗口逐基金最近 20 交易日）」——**只修文字，不改计算**，WF 数字与 verdict 不动；单基金口径归线上 `shadow_policy` / `forecast_path(fund_code=)` 使用，两处差异自此可在源码上一眼对账。顺带修 `str(None)[:12] or 'unknown'` 显示缺陷（会印 "None"），抽 `_code_commit_short()` 先判空再截。测试 606→613（+6 测例：status 读取、缺失/损坏→unknown、stale 标记随证据出、陈旧不改 confidence、commit 显示、报告陈旧行）。⑤ **离线冒烟抽到一个既存隐患**（`_read_wf_evidence`）：WF md 现含四张表，`| T+5 |` / `| 4 |` 前缀**跨表重复**——旧解析只按前缀匹配，把 CQR 覆盖率行（cells[1]="84.2%"）当 IC 行直接 `ValueError` 崩卡，也会把 Path-WF 折表（11 列）的 μ/σ 当 IC 静默错解析；改按**列数认表**（主判据 4 列 / 逐折 6 列，与 09-01 `_read_calib_evidence` 同风格）+1 回归测例（合成四表互扰件；本文件属 fast 层，不新增读真实 output/ 的用例）。验收：fast 457 + slow 156 = 613 OK(skipped=3) 零回归；`backtest_t5_scorecard.py --force` 离线端到端跑通（exit=0，⑨ 行实输出 `⚠️ STALE（09-26 须重跑） | lofo json (status=STALE)`，裁决 fail 与本卡重算 -0.0790 一致）
- V4.4-步1: **14:55 PIT Forecast 契约 + 重算构建器（2026-09-22，Summer 拍板 label 分母口径）**（① label 分母改「14:55 可观测状态」：现行 `fwd=navs[T+h]/navs[T]-1` 的分母是 T 日**最终** NAV，而 14:55 决策时当日 NAV 未公布（约 21~22 点出）→ 以不可见价为基准语义不纯。新口径 `nav_hat_1455=navs[T-1]×(1+est_chg)`、`fwd_from_1455=navs[T+h]/nav_hat_1455-1`，契约单一真源 `core/pit1455_contract.py`（纯函数）。② 顺带钉死 **est_chg 量纲 = fraction** 并核出一个独立硬 P0：训练/冻结样本用 `closes` 比值（小数，absmax 0.1798），live/shadow 用腾讯行情 `parts[32]`（**百分数**，72 条 post 行 absmax 7.89）⇒ 同一 key 差 100×，**66/72 = 91.7% 的 live 行落在训练支撑域外**（/100 归一后越界归零）。本步只登记唯一换算桥 `est_chg_from_pct()`，**不改 run.py/shadow 产出**（接入是步 2，届时禁止各消费点就地 *0.01/÷100）。③ `pit1455_dataset.py`（零网络只读）逐行套契约重算 → `forecast_outputs/samples_pit1455_20260922.jsonl` + meta：3371/3371 零跳过，原字段全留、新 label 以 `*_1455` **并存不原位覆盖**（可对照可回滚）；抽行手工复算一致。④ **步 3 基准必须换**：`P(fwd>0)` 旧→新 fwd3 51.38%→51.20%、fwd5 52.80%→51.97%——旧口径把 T 日尾盘涨幅记进分母，人为抬高中短周期「上涨」基线，再拿 52.8% 当多数类会系统性高估模型命中。⑤ 现有 `fwd*` 旧 label 一字未动（12 个回测入口口径不变，零回归）；新增 `tests/test_pit1455_contract.py` 8 项入 fast（含黄金校验：est_chg 取 T 日真实涨跌时新 label 必退化为原 label，锁死「分母取 T-1、索引 i+h 对齐」）。fast 463→471、全量 627 OK(skipped=3) 零回归（注：README 上一条记的 613 之后，前批评审修复 `672b9ed` 给 test_intraday 加了 6 项未同步 README，故本步起点为 619 而非 613）
- V4.4-步2: **est_chg 量纲接入 + 可复现验证重跑（2026-09-22，Summer 授权执行）**（① **接入**：live 侧 est_chg 全部过唯一桥 `est_chg_from_pct` 消除 100× 错位——`run.py build_forecast_meta`（est_return 百分数→fraction 特征）、`core/decision_engine._forecast_features`（fallback 路径同桥），无任何消费点就地 *0.01/÷100。② **量纲迁移判别**：store 只追加不覆写 ⇒ `data/intraday_features.jsonl` 新旧量纲并存，契约新增切换日 `EST_CHG_LIVE_FRACTION_SINCE="2026-09-23"` + 判别函数 `est_chg_live_is_fraction()`；读取端 `shadow_policy.load_post_inputs` / `score_expert_a.load_post_features` 对早于切换日的行过桥归一、之后透传。③ **% 域消费点还原**：shadow `mirror_relative_score` 经逆向桥 `est_pct_from_fraction` 还原百分数再喂 `_score_relative`（其 est_return 语义是 %），mirror 块新增 `est_chg_fraction` 原值字段（审计可对），Expert A 臂打分矩阵（m0.matrix_existing 含 est_chg）自此吃到正确 fraction。④ **不 bump MODEL_VERSION / 不重训替换生产权重**（裁量留痕）：训练/冻结样本本就是 fraction（closes 比值），错在 live 输入侧，接入后 live 与现有 v3 模型自然对齐；且 `data/promotion_prereg.json`（Summer 09-13 亲批）将 `forecast_v3.pkl` sha 与报告 sha 钉死至 2026-10-12，重训换名即当场断今晚 22:30 shadow 的 prereg_degraded 授权链。「重训出 v4」留待 Summer 拍板（需同步改 prereg 授权与新冻结件重训）。⑤ **可复现验证重跑**：`backtest_forecast.py` 消费 0910 冻结件（G-A PASS sha=be8e55f02b39，G-B DRIFTED 软标记如实入报告）→ `output/backtest_forecast_v44_step2_20260922.log`，两次运行字节级一致（fc 无差异）；裁决 T+1/T+3/T+5 全 ❌（rank_ic +0.049/+0.048/−0.019，CI 全跨零；n_oos=923），**维持 model_ready=false 与 v3 权重不动**——该 ❌ 系 0910 冻结基线与 09-08 `--fresh` 历史报告不可比（DRIFTED），非本步回归（训练侧代码一字未动）。报告仅留档证据，**未做 bind_validation**（绑定会把 registry 里 v3 的 validation 报告哈希覆盖掉，弄断 prereg D3——补 P1-2 报告与「v3 现行授权」解耦，留待 Summer 处置）。⑥ 测试：契约桥/切换日门 4 项、run 特征换算 1 项、shadow 迁移+镜像分 4 项；fast 471→477、slow 158 OK(skipped=3)（.venv 跑，系统 python 无 sklearn 会在 slow 层误报）。全程零东财请求：训练/验证走冻结快照，G-B 指纹重算只读本地缓存，铁律 7 四闸门以「无请求」满足）
...
### 历史特征时点口径声明（EOD_PROXY · 2026-09-23）

**结论先行**：历史回测样本中，「14:55 决策时点」的个股特征是**用当日收盘价（EOD close）近似的**，
不是真实 14:55 快照。历史结论在此口径下成立，须按 `EOD_PROXY` 解读，不得声称「严格 14:55 重建」。

| 维度 | 现状 |
|---|---|
| 实现位置 | `backtest_spread.py` 的个股估涨：`j = bisect_right(dates, d) - 1` 取到 **d 日收盘**，等价于用当日 EOD close 当 14:55 价 |
| 残差量级 | 有界——只差「14:55 → 收盘」的尾盘漂移，不是跨日错位 |
| NAV 系因子 | 不在此列：`_nav_state_at` 已统一错位至 T-1 公布口径（PIT 正确） |
| 持仓披露滞后 | 不在此列：`_QTR_LAG` 保守下界已在 `check_pit()` 独立审计（P0-1），属另一维度 |
| 影响边界 | **数值层零变化**——本声明是口径诚实化，不改任何既有回测产出数字 |
| 冻结件 | `samples_frozen_20260910.jsonl` **本体不改**（冻结即冻结），仅在文档与**今后新生成**的样本 meta 里标注 |
| 真实 14:55 快照 | 2026-09-23 起每槽落盘攒训练轨；历史分钟线回补需另行授权查证 |

样本 meta 侧新增字段 `historical_feature_mode`（枚举：`EOD_PROXY` / `PIT_1455_SNAPSHOT`），
已存在的 0910 冻结件 meta **不追溯改写**（改它等于伪造冻结事实）；新生成的样本件一律带此字段。

**历史分钟线回补探测方案（2026-09-23 起草，零执行）**：缺口定义、回补目标刻画（回补唯一外部依赖 =
每只重仓股在历史交易日「截至 14:55 涨跌幅」）、七条探测问题、G0~G4 步骤与纪律边界，
见 `output/ops_runs/2026-09-23-probe-intraday-backfill-plan.md`。**该文件是方案不是执行记录**：
G1/G2/G3 涉及外部请求，须 Summer 显式授权并过铁律 7 四项闸门；G0 为零网络本地取证，可先行。
