...
v4 增强（2026-08-25，GPT 诊断第十三节）：
- positions 新增 position_pct（单基金市值占比）→ 决策引擎「账户允许」门槛输入
- holdings.json 支持可选字段 max_position_pct（单基金仓位上限，缺省 0.8）/
  consecutive_adds（连续加仓次数，缺省 0）
- 组合集中度 HHI = Σ(单基金占比²) 作为账户面参考指标
...
def evaluate_account(signals: dict[str, dict]) -> dict:
    ...
    for code, h in holdings.items():
        ...
        positions.append({
            ...
            "max_position_pct": h.get("max_position_pct", 0.8),
            "consecutive_adds": h.get("consecutive_adds", 0),
        })
        total_mv += mv
        total_cost += h["shares"] * (cost or nav)

    # v4：组合集中度 HHI = Σ(单基金占比²)，越高越集中；position_pct 供决策引擎门槛
    for p in positions:
        p["position_pct"] = round(p["market_value"] / total_mv, 4) if total_mv > 0 else 0.0
    hhi = sum(p["position_pct"] ** 2 for p in positions) if total_mv > 0 else 0.0
